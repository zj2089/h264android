...
#ifdef BREW
typedef void (*ChromaFullMC_SIMD_Func)(uint8 *pRef, int srcPitch, int dx, int dy,
									   uint8 *pOut, int predPitch, int blkwidth, int blkheight);

void Init_pred_inter_ChromaMC_SIMD()
{
	ChromaFullMC_SIMD_Func tmp_ChromaMC_SIMD[8];
	tmp_ChromaMC_SIMD[0] = &ChromaFullMC_SIMD;
	tmp_ChromaMC_SIMD[1] = &ChromaHorizontalMC_SIMD;
	tmp_ChromaMC_SIMD[2] = &ChromaVerticalMC_SIMD;
	tmp_ChromaMC_SIMD[3] = &ChromaDiagonalMC_SIMD;
	tmp_ChromaMC_SIMD[4] = &ChromaFullMC_SIMD;
	tmp_ChromaMC_SIMD[5] = &ChromaHorizontalMC2_SIMD;
	tmp_ChromaMC_SIMD[6] = &ChromaVerticalMC2_SIMD;
	tmp_ChromaMC_SIMD[7] = &ChromaDiagonalMC2_SIMD;

	GLOBAL_VARIBALS->pred_inter_ChromaMC_SIMD = malloc(sizeof(tmp_ChromaMC_SIMD));
	memcpy(GLOBAL_VARIBALS->pred_inter_ChromaMC_SIMD, tmp_ChromaMC_SIMD, sizeof(tmp_ChromaMC_SIMD));
}

void Destroy_pred_inter_ChromaMC_SIMD()
{
	free(GLOBAL_VARIBALS->pred_inter_ChromaMC_SIMD);
	GLOBAL_VARIBALS->pred_inter_ChromaMC_SIMD = 0;
}
#define ChromaMC_SIMD ((ChromaFullMC_SIMD_Func*)(GLOBAL_VARIBALS->pred_inter_ChromaMC_SIMD))
#else
                            ...
                            uint8 *pOut,  int predPitch, int blkwidth, int blkheight)
{
    ...
    for (i = 0; i < blkheight; i++)
    {
        ...
        result >>= 6;
        result &= 0x00FF00FF;
        *(int16 *)pOut = (int16)( (result >> 8) | (result & 0xFF) );
        r0 = r1;
        pOut += predPitch;
    }
    return;
